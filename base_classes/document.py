...
#         result.append("url: "+ self.url + NEW_LINE);
        ...
        if (self.page_rank != 0): result.append("page_rank: " + str(self.page_rank) + NEW_LINE);
        if (self.anchors is not None): result.append("anchors: " + str(self.anchors) + NEW_LINE);
        if (self.body_content is not None): result.append("body_content: " + str(self.body_content[:100]) + NEW_LINE);
        result.append("relevance: " + str(self.relevance) + NEW_LINE)
        return " ".join(result)
